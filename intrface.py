...
def main():
    ...
    if(selected=='Disease Prediction'):
        ...
        selected_option1 = st.selectbox("Select 1st symptom:", list(l1.keys()))
        selected_option2 = st.selectbox("Select 2nd symptom:", list(l2.keys()))
        selected_option3 = st.selectbox("Select 3rd symptom:", list(l3.keys()))
        selected_option4 = st.selectbox("Select 4th symptom:", list(l4.keys()))
        selected_option5 = st.selectbox("Select 5th symptom:", list(l5.keys()))
        selected_option6 = st.selectbox("Select 6th symptom:", list(l6.keys()))
        ...
        if st.button('Predicted Disease'):
            selected_options = [selected_option1, selected_option2, selected_option3, selected_option4, selected_option5, selected_option6]
            data1 = {'selected_option1': selected_options[0], 'selected_option2': selected_options[1],'selected_option3': selected_options[2], 'selected_option4': selected_options[3],'selected_option5': selected_options[4],'selected_option6': selected_options[5] }
            response = requests.post('http://localhost:8501/intrface', json={'data': data1})
            
   
            st.write("Diseases Selected by you are : ",data1)

            for opt,symptom in data1.items():
                index = d1.get(symptom)
                if index is not None:
                    data[index] = 1
            st.write("The input array for the Algorithm: ",data)

            predicted_diseaseRF=predict_diseaseRF(data)
            st.write("Predicted Disease via RF Algorithm: ", predicted_diseaseRF)
            
            predicted_diseaseMLP=predict_diseaseMLP(data)
            #st.write("Predicted Disease via MLP Algorithm: ", predicted_diseaseMLP)
            if response and response.status_code == 200: 
                result = response.json()
                predicted_disease=predict_diseaseRF(result)
...
def predict_diseaseRF(data):
   
    trained_RF=jb.load('clfRF_model.pkl')
    predicted_disease = trained_RF.predict([data])[0]
    
    return predicted_disease

            
def predict_diseaseMLP(data):
   
    trained_MLP=jb.load('clfMLP_model.pkl')
    predicted_disease = trained_MLP.predict([data])[0]
